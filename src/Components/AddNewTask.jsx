...
const AddNewTask = ({addStatus, onClose, onTaskCreated}) => {
  ...
  async function handleCreateTask(e){
    ...
    try{
      ...
      // so far does nothing with received data
    ...
    <div onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-black/65 px-4 backdrop-blur-[2px]">
      <form onClick={(e) => e.stopPropagation()} onSubmit={handleCreateTask} className="w-full max-w-lg rounded-2xl border border-slate-800 bg-[#0F1626] p-5 shadow-2xl shadow-black/40 sm:p-7">
        ...
        <div className="mb-4 flex flex-col gap-2">
          ...
          <input id="title" name="title" placeholder="Enter task title" type="text" ref={titleRef} className="h-11 rounded-lg border border-slate-700 bg-[#0B1020] px-4 text-sm text-slate-100 placeholder:text-slate-500 outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20"/>
        </div>
        {/* Description */}
        <div className="mb-4 flex flex-col gap-2">
          <label htmlFor="description" className="text-sm font-medium text-slate-300">Description</label>
          <textarea id="description" name="description" placeholder="Enter task description" ref={descRef} rows="3" className="resize-none rounded-lg border border-slate-700 bg-[#0B1020] px-4 py-3 text-sm text-slate-100 placeholder:text-slate-500 outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20" />
        </div>
        {/* Status */}
        <div className="mb-4 flex flex-col gap-2">
          <label htmlFor="status" className="text-sm font-medium text-slate-300">Status</label>
          <select id="status" name="status" ref={statusRef} defaultValue={addStatus} className="h-11 cursor-pointer rounded-lg border border-slate-700 bg-[#0B1020] px-4 text-sm text-slate-100 outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20">
        ...
        <div className="mb-6 flex flex-col gap-2">
          ...
          <select id="priority" name="priority" ref={priorityRef} defaultValue="" className="h-11 cursor-pointer rounded-lg border border-slate-700 bg-[#0B1020] px-4 text-sm text-slate-100 outline-none transition focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20" >
        ...
        <div className="flex items-center gap-3">
          ...
          <button type="submit" className="h-11 flex-[2] cursor-pointer rounded-lg bg-violet-600 px-4 text-sm font-semibold text-white shadow-lg shadow-violet-950/30 transition hover:bg-violet-500 active:translate-y-px">Create Task</button>
        </div>
      </form>
    </div>
  )
